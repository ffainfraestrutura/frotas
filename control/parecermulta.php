...
if (!$conn instanceof mysqli || $databaseName === '') {
    header('Location: ../multa/multasfrota.php');
    exit;
}
...
if ($etapa !== '') {
    [$tramite, $tdp, $tfin, $tsup, $status] = $mapaEtapa[$etapa] ?? ['Imprimir Recibo DP', '', '', '', 1];
...
$stmt = mysqli_prepare($conn, "INSERT INTO tbmulta_parecer (`parecer`, `matricula`, `data_parecer`, `id_multa`) VALUES (?, ?, NOW(), ?);");
