...
function renderSelect(string $name, string $label, array $options, string $valueKey = '', string $textKey = '', bool $required = false, string $extraClass = '', string $placeholder = 'Selecione...'): void
{
    ...
    foreach ($options as $valor => $option) {
        ...
        if ($value !== '') {
            echo '<option value="' . esc($value) . '">' . esc($text) . '</option>';
        }
    }
            ...
            <form method="post" action="control/cadastrarveiculo.php" enctype="multipart/form-data" class="pb-5">
                ...
                <div class="card mb-3">
                    ...
                    <div class="card-body row g-3">
                        ...
                        <?php renderSelect('matcond', 'Condutor', $funcionariosFormatados, 'matricula', 'descricao'); ?>
