...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST' && $placa !== '') {
    header('Location: dadosplaca.php?' . http_build_query(['placa' => $placa]));
    exit;
}
$veiculo = [];
$condutor = [];
...
if ($placa !== '' && isset($conn) && $conn instanceof mysqli) {
    ...
    if ($veiculo !== []) {
        $matriculaCondutor = valorLinha($veiculo, ['matcond', 'matcondutor']);
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="d-flex gap-2">
            <a class="btn btn-secondary" href="/autofrota/veiculos/listagem-veiculo.php"><i class="fa fa-list me-1"></i>Listagem de Veículos</a>
            <a class="btn btn-primary" href="/autofrota/veiculos/inventario-veiculo.php"><i class="fa fa-car me-1"></i>Todos Veículos</a>
            <a class="btn btn-info" href="/autofrota/condutores/listagem-condutor.php"><i class="fa fa-id-card me-1"></i>Listagem de Condutores</a>
        </div>
    </div>
    ...
    <?php else: ?>
        ...
        <div class="d-flex flex-wrap gap-2">
            <a class="btn btn-success" href="historicoplacachecklist.php?placa=<?= urlencode($placa) ?>">Histórico de Vistorias</a>
            <a class="btn btn-secondary" href="historico-placas-condutor.php?placa=<?= urlencode($placa) ?>">Histórico de Condutores</a>
            <a class="btn btn-primary" href="historicoplacamanutencao.php?placa=<?= urlencode($placa) ?>">Histórico de Manutenções</a>
        </div>
    <?php endif; ?>
</div>
