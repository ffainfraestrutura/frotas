...
function montarLinhaVeiculo(array $veiculo, DateTimeImmutable $hoje, string $perfilLogado, string $matriculaLogada, bool $bloquearEdicao): array
{
    ...
    $infoPlaca = montarBotaoPost('historicoplaca.php', [
        'placa' => $placa,
        'perfil' => $perfilLogado,
        'matr_autor' => $matriculaLogada,
    ...
    if ($matCond !== '') {
        $infoCondutor = montarBotaoPost('dadoscondutor.php', [
            'matcond' => $matCond,
    ...
    if ($podeExibirAcoes) {
        if ($emManutencao) {
            ...
            $manutencaoHtml = montarBotaoPost('manutencoes/iniciocadastromanpreventiva.php?placa=' . rawurlencode($placa), $camposAutor + [
                'placa' => $placa,
            ], 'add_circle', 'Cadastrar nova manutenção');
        }

        $editarHtml = montarBotaoPost('editarcadveiculo.php', $camposAutor + [
            'idtbveiculo' => $idVeiculo,
        ], 'edit_note', 'Editar Veículo');

        $documentosHtml = montarBotaoPost('enviardocumento.php', $camposAutor + [
            'idtbveiculo' => $idVeiculo,
            'placa' => $placa,
        ], 'attach_file', 'Enviar Documentos');

        $apagarHtml = '<button type="button" data-bs-toggle="modal" data-bs-target="#apagarveic" '
            . 'onclick="popupApagarVeiculo(' . json_encode($idVeiculo, JSON_HEX_APOS | JSON_HEX_QUOT) . ', ' . json_encode($placa, JSON_HEX_APOS | JSON_HEX_QUOT) . ')" '
...
$statusAtivoSelecionado = ($_POST['statusa'] ?? '') === '1';
$statusInativoSelecionado = ($_POST['statusi'] ?? '') === '0';
$statusSelecionados = [];
...
<body class="sb-nav-fixed">
        ...
        <main class="page-wrapper py-3">
            ...
            <section class="filter-area">
                <form action="veiculos.php" method="post">
                    <div class="mb-2" style="max-width: 445px;">
                    ...
                    <div class="mb-2">
                        ...
                        <div class="form-control d-flex flex-wrap gap-2" id="bases">
                            <?php foreach ($basesGestao as $base): ?>
                                <?php $baseNome = (string) ($base['basegestao'] ?? ''); ?>
                                <?php if ($baseNome !== ''): ?>
                                    <label class="me-1 <?= esc($base['unidade'] ?? '') ?>">
                                        <input type="checkbox" name="bases[]" value="<?= esc($baseNome) ?>" <?= in_array($baseNome, $basesSelecionadas, true) ? 'checked' : '' ?>>
                                        <?= esc($baseNome) ?>
                                    </label>
                                <?php endif; ?>
                            <?php endforeach; ?>
                    ...
                    <div class="mb-2">
                        ...
                        <div class="form-control d-flex flex-wrap gap-2" id="status">
                            <label class="me-1"><input type="checkbox" name="statusa" value="1" <?= $statusAtivoSelecionado ? 'checked' : '' ?>> Ativo</label>
                            <label class="me-1"><input type="checkbox" name="statusi" value="0" <?= $statusInativoSelecionado ? 'checked' : '' ?>> Inativo</label>
                        </div>
                    </div>
            ...
            <section class="d-flex justify-content-start actions flex-wrap align-items-center">
                ...
                <form action="control/gerarexcelveicativoscompleto.php" method="post" target="_blank" class="d-inline">
                    ...
                    <button type="submit" class="btn btn-excel-green">Gerar Excel - Veículos Ativos</button>
                </form>
            </section>
    ...
    <script>
        ...
        function popupApagarVeiculo(idtbveiculo, placa) {
