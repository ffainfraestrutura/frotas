...
<div class="d-flex justify-content-end gap-2 mb-3 no-print"><button class="btn btn-outline-secondary" type="button" onclick="if (window.history.length > 1) { window.history.back(); } else { window.location.href = './checklistinicio.php'; }"><i class="fas fa-arrow-left me-1"></i>Voltar</button><button class="btn btn-primary" type="button" onclick="window.print()"><i class="fas fa-print me-1"></i>Imprimir</button></div>
...
<section class="card report-card mb-4 page-break" id="fotos-vistoria"><div class="card-header bg-white"><h2 class="h5 mb-0">Fotos da vistoria</h2></div><div class="card-body row g-3"><?php $temFoto=false; foreach ($camposFotos as $campo=>$rotulo): if (empty($fotos[$campo])) continue; $temFoto=true; ?><figure class="col-md-6 mb-0"><figcaption class="fw-bold mb-2"><?= $e($rotulo) ?></figcaption><img class="photo" src=".<?= $e($fotos[$campo]) ?>" alt="<?= $e($rotulo) ?>"></figure><?php endforeach; if (!$temFoto): ?><div class="col-12 text-muted">Nenhuma foto foi registrada.</div><?php endif ?></div></section>
