...
if ($supervisorTabela === '') {
    ...
    if (($consulta['erro'] ?? '') !== '') {
        ...
        if ($perfilLogado !== '3' && $pedidos !== []) {
            ...
            foreach ($pedidos as $indice => $pedido) {
                $deveEscalonar = false;
                $saldoCartao = $pedido['saldo_cartao_api'] ?? $pedido['sldcartao'] ?? 0;
                if ((float) $saldoCartao > $limiteSaldoEscalonamento) {
                    $deveEscalonar = true;
                }
    ...
    <main class="page-wrapper py-2">
        <section class="card panel-card mb-4">
            <div class="card-body d-flex flex-column flex-lg-row justify-content-between gap-3">
                <div>
                    <h1 class="h3 mb-1">Aprovação de pedidos de Cota Extra</h1>
                    <p class="text-muted mb-0">Coordenador: <strong><?= escCota($nomeExibicao . '(' . $matriculaLogada . ')') ?></strong></p>
                </div>
                </div>
            </div>
        </section>
