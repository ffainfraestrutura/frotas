...
if ($id > 0 && $con instanceof mysqli && preg_match('/^[A-Za-z0-9_]+$/', $databaseName) === 1) {
    ...
    if ($vistoria) {
        ...
        $stmtStatus = mysqli_prepare($con, "SELECT status FROM `{$databaseName}`.`tbastatusvel` WHERE idtbastatusvel = ? LIMIT 1");
        if ($stmtStatus) {
...
<section class="card report-card mb-4"><div class="card-header bg-white"><h2 class="h5 mb-0">Itens completos da vistoria</h2></div><div class="card-body"><?php foreach ($gruposItens as $grupo=>$itens): ?><h3 class="h6 border-bottom pb-2 mt-3"><?= $e($grupo) ?></h3><div class="row g-2"><?php foreach ($itens as $campo=>$rotulo): ?><?php $valor=$rotuloItem($vistoria[$campo] ?? ''); ?><div class="col-md-4"><strong><?= $e($rotulo) ?>:</strong> <span class="<?= $valor === 'OK' ? 'item-ok' : ($valor === 'NÃO OK' ? 'item-bad' : 'text-muted') ?>"><?= $e($valor) ?></span></div><?php endforeach ?></div><?php endforeach ?></div></section>
