...
$resultad = mysql_query($sql) or die (mysql_error());
    ...
    $sql2 =" SELECT count(*) as totalsaida, (src) as codagente, round(AVG(billsec),2) as media, date(calldate) as calldate 
             ...
             where LENGTH(dst) > 7 and calldate >= '" . $di . "' 
             and calldate <= '" . $df . "' 
...
//echo$sql2;
  ...
  $sql = " select count(*) as totalentrada, MID(agent, 7,4) as agente, round(AVG(data2),2) as media, date(time) as calldate
               from asteriskcdrdb.queue_log where time >= '" . $di . "' and time <= '" . $df . "'  
	           and length(MID(agent, 7,4)) > 3
