    ...
    if (in_array($file_extension, $allowed_extension)) {
        ...
        foreach ($data as $rowIndex => $row) { //aqui se faz o looping
            ...
            if (!empty($ccustoplan)) { // Corrected variable from $ccusto to $ccustoplan
                $sql3 = "SELECT ccusto FROM tbfuncionario WHERE ccusto = '" . mysqli_real_escape_string($con, $ccustoplan) . "';";
            ...
            if (!$gravar) {
                ...
                if (!empty($nomecond)) {
                    ...
                    $sql_cond_placa = "SELECT C.nome, C.matricula, F.idtbfuncionario, F.codfilial, F.codempresa, F.cpf, F.ccusto
                                       ...
                                       LEFT JOIN tbfuncionario F ON C.matricula = F.matricula  -- Assuming matricula links them
                ...
                if ($codempresa && $codfilial) {
                    $sql3 = "SELECT nome FROM tbfilial WHERE codempresa='" . mysqli_real_escape_string($con, $codempresa) . "' AND codfilial='" . mysqli_real_escape_string($con, $codfilial) . "'; ";
                    $resultado3 = mysqli_query($con, $sql3);
